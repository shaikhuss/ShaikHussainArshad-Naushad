...
namespace Azul
{
	...
	void TextureMan::Destroy()
	{
		...
		TextureLink *pLink = pTextMan->pActive;

		while (pLink != nullptr)
		{
			TextureLink *pTmp = pLink;
			pLink = pLink->next;
			pTextMan->privRemove(pTmp, pTextMan->pActive);
			delete pTmp;
		}
	}
	...
	void TextureMan::Add(const char *const _assetName, const Texture::Name _name)
	{
		...
		pTextMan->privAddToFront(pNode, pTextMan->pActive);
	}
	...
	GLuint TextureMan::FindID(const Texture::Name _name)
	{
		...
		Texture *pNode = (Texture *)pTextMan->pActive;
		while (pNode != nullptr)
		{
			if (pNode->name == _name)
			{
	...
	void TextureMan::AddSpu(const char* const _assetName, const Texture::Name _name)
	{
		unsigned char* pTextData;
		unsigned int chunkSize;
		eat(_assetName, ChunkType::NORMS_TYPE, "", pTextData, chunkSize);
		...
		pTextMan->privLoadSpuTexture(pTextData, pTextureID);

		Texture* pNode = new Texture();

		// initialize it
		pNode->set(_assetName, _name, textureID, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, -1, -1);

		// Now add it to the manager
		pTextMan->privAddToFront(pNode, pTextMan->pActive);

	}
	void TextureMan::privLoadSpuTexture(unsigned char* _assetName, GLuint*& textureID)
	{
		...
		// Loat the texture
		this->privLoadSpuTGATexture(_assetName, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
	}
	...
	bool TextureMan::privLoadSpuTGATexture(unsigned char* szFileName, GLint minFilter, GLint magFilter, GLint horizWrapMode, GLint vertWrapMode)
	{
		GLbyte* pBits = (GLbyte*)szFileName;
		int nWidth, nHeight, nComponents = 0;
		GLenum eFormat = 0;
		short sDepth;

		TGAHEADER tgaHeader = *(TGAHEADER*)szFileName;
	...
	GLbyte* TextureMan::gltReadSpuTGABits(unsigned char* szFileName, GLint* iWidth, GLint* iHeight, GLint* iComponents, GLenum* eFormat)
	{
		...
		*iComponents = GL_RGB;
		...
		fopen_s(&pFile, (const char*)szFileName, "rb");
